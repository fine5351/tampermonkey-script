...
(function () {
    ...
    async function waitForElement(stepName, selector, filterFn = (el) => true) {
        while (true) {
            ...
            for (let el of elements) {
                if (filterFn(el)) {
                    console.log(`[Bilibili Festival F8] 找到目標元素: ${selector}, 執行行為: ${stepName}`);
                    return el;
                }
            }
            console.log(`[Bilibili Festival F8] 步驟 : ${stepName}, 持續尋找中 ${selector}...`);
    ...
    async function runIframeLogic() {
        console.log('[Bilibili Festival F8] Running inside share iframe.');
    ...
    async function clickLike() {
        await waitAndClick('點贊', 'div.video-toolbar-content_item', (el) => el.querySelector('.ic_like') !== null);
    }

    // 2. 收藏到默認收藏夾
    async function clickFavorite() {
        // A. 點擊收藏按鈕
        await waitAndClick('開啟收藏彈窗', 'div.video-toolbar-content_item', (el) => el.querySelector('.ic_collection') !== null);
        ...
        if (checkbox && !checkbox.checked) {
            console.log('[Bilibili Festival F8] 執行行為: 勾選默認收藏夾');
            checkbox.click();
            await new Promise(r => setTimeout(r, 500));
        } else {
            console.log('[Bilibili Festival F8] 默認收藏夾已勾選，跳過');
        }
    ...
    async function clickShareToDynamic() {
        // A. 移入分享按鈕
        const shareBtn = await waitForElement('移入分享按鈕', 'div.video-toolbar-content_item.share');
        console.log('[Bilibili Festival F8] 執行行為: hover 分享按鈕');
        shareBtn.dispatchEvent(new MouseEvent('mouseenter', { bubbles: true, cancelable: true }));
        await new Promise(r => setTimeout(r, 500));

        // B. 點擊「動態」
        await waitAndClick('點擊分享到動態', 'span.share-btn', (el) => el.textContent.includes('动态'));
    ...
    async function monitorShareDialog(shareBtn) {
        ...
        while (document.body.contains(iframe)) {
            console.log('[Bilibili Festival F8] 步驟 : 等待分享完成 (iframe 消失), 持續監測中...');
            await new Promise(r => setTimeout(r, 1000));
        }

        console.log('[Bilibili Festival F8] 找到目標元素: iframe 消失, 執行行為: 關閉分享菜單');
    ...
    document.addEventListener('keydown', async function (e) {
        if (e.key === 'F8') {
            console.log('[Bilibili Festival F8] 開始執行三連流程...');
            try {
                await clickLike();
                await clickFavorite();
                await clickShareToDynamic();
                console.log('[Bilibili Festival F8] 三連流程全部執行完畢！');
            } catch (err) {
                console.error('[Bilibili Festival F8] 流程中斷:', err);
            }
        }
    });
})();
